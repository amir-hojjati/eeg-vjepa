...
def main(args_eval, resume_preempt=False):
    ...
    supervised = args_data.get('supervised', False)
    ...
    # Using same number as pretrain frames_per_clip
    ## eval_frames_per_clip = args_data.get('frames_per_clip', 16)
    eval_frames_per_clip = args_pretrain.get('frames_per_clip', 16)
    ...
    logger.info(f'Fully supervised: {supervised}')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        train_auroc, train_auprc, train_f1 = calculate_metrics(train_preds, train_labels)
        val_auroc, val_auprc, val_f1 = calculate_metrics(val_preds, val_labels)


        logger.info(
            '[%5d] train: %.3f%% test: %.3f%% train_auroc: %.3f val_auroc: %.3f train_auprc: %.3f val_auprc: %.3f train_f1: %.3f val_f1: %.3f' % (epoch + 1, train_acc, val_acc, train_auroc, val_auroc, train_auprc, val_auprc, train_f1, val_f1))
        if rank == 0:
            csv_logger.log(epoch + 1, train_acc, val_acc, train_auroc, val_auroc, train_auprc, val_auprc, train_f1, val_f1)
...
def run_one_epoch(
    ...
    for itr, data in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.float16, enabled=use_bfloat16):
            ...
            labels = torch.cat([u.to(device, non_blocking=True) for u in data[1]], dim=0)
            ...
            with torch.no_grad():
                ...
                if (not supervised) or (supervised and (not training)):
    ...
    return top1_meter.avg, torch.cat(all_preds), torch.cat(all_labels)
...
def calculate_metrics(all_preds, all_labels):
    ...
    preds = (all_preds[:, 1] > 0.5).float()  # Convert to binary predictions

    # Calculate metrics for each class
    f1_scores = []
    for class_idx in range(2):
        true_positives = torch.sum((preds == class_idx) & (all_labels == class_idx))
        false_positives = torch.sum((preds == class_idx) & (all_labels != class_idx))
        false_negatives = torch.sum((preds != class_idx) & (all_labels == class_idx))
    ...
    # Calculate AUROC and AUPRC as before
    sorted_preds, sort_idx = torch.sort(all_preds[:, 1], descending=True)
    ...
    precision = torch.cumsum(sorted_labels, dim=0) / torch.arange(1, len(sorted_labels) + 1,
                                                                  device=sorted_labels.device)
    recall = torch.cumsum(sorted_labels, dim=0) / torch.sum(sorted_labels)
    auprc = torch.trapz(precision, recall)

    return auroc.item(), auprc.item(), macro_f1
